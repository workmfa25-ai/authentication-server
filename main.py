# # Endpoint: Monthly Logins (per day for a given month)
...
# # @app.get("/admin/analytics/monthly-logins")
# # async def monthly_logins(
# #     year: int = Query(..., ge=1970, le=2100),
# #     month: int = Query(..., ge=1, le=12),
# #     source: str = Query("grid", pattern="^(grid|jwt)$"),
...
# #     # model selection
# #     model = GridSession if source == "grid" else JwtSession
...
# #     async with async_session() as session:
# #         # group by day using cast to Date (portable for most DBs)
# #         stmt = (
# #             select(cast(model.created_at, Date).label("day"), func.count(model.id))
# #             .where(and_(model.created_at >= start, model.created_at < end))
# #             .group_by("day")
# #             .order_by("day")
# #         )
# #         result = await session.execute(stmt)
# #         rows = result.all()  # list of (date_obj, count)

# #     # build counts array indexed by day
# #     counts_by_day = {d.day: c for d, c in rows}
# #     counts = [counts_by_day.get(day, 0) for day in range(1, days_in_month + 1)]

# #     # Nivo-friendly series
# #     import calendar
# #     month_short = calendar.month_abbr[month]
# #     series = [
# #         {"x": day, "y": counts[day - 1], "label": f"{month_short} {day}, {year}"}
...
# #         "total": sum(counts),
# #         "counts": [{"day": i + 1, "count": c} for i, c in enumerate(counts)],
# #         "series": series,  # useful directly for Nivo
# #         "source": source,
# #         "monthTitle": f"{calendar.month_name[month]} {year}"
# #     }
...
#     async with async_session() as session:
#         async with session.begin():  # this manages commit/rollback automatically
#             new_user = User(
...
#         return {
#             "username": username,
#             "available": db_user is None  # True if not found
#         }
...
# async def list_jwt_sessions(skip: int = Query(0, ge=0), limit: int = Query(10, ge=1, le=100)):
#     async with async_session() as session:
...
# class GridChallengeRequest(BaseModel):
#     username: str

# class GridChallengeResponse(BaseModel):
#     grid: dict
#     signature: str
#     challenge_coords: list

# class GridValidateRequest(BaseModel):
#     username: str
#     signature: str
#     answers: dict  # e.g., {"A1": "28", "C3": "45", "E5": "67"}
